...
describe('fromPairs', () => {
  ...
  it('测试包含不同类型的键值对', () => {
    ...
    ).toEqual({ key1: 'value1', key2: 2, key3: true })
  })
...
describe('unique', () => {
  ...
  it('测试包含重复对象的数组-非深度', () => {
    expect(unique([1, 2, 1, { id: 1 }, { id: 2 }, { id: 1 }], { deep: false })).toEqual([
      1,
      2,
      { id: 1 },
      { id: 2 },
  ...
  it('测试包含重复键的对象数组', () => {
    expect(unique([{ name: 'Alice' }, { name: 'Bob' }, { name: 'Alice' }], { key: 'name' })).toEqual([
      { name: 'Alice' },
      { name: 'Bob' }
    ])
  })
  ...
  it('测试包含嵌套对象的数组', () => {
    expect(unique([{ user: { id: 1 } }, { user: { id: 2 } }, { user: { id: 1 } }], { key: 'user' })).toEqual([
      { user: { id: 1 } },
      { user: { id: 2 } }
    ])
  })
