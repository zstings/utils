...
## copy
复制文本内容 优先使用navigator.clipboard.writeText, 浏览器不支持使用时降级document.execCommand。
#### 类型说明
::: info
...
## random
均衡获取指定范围的随机整数 返回一个startNum 和 endNum之间的随机整数。如果你没有参数，那么将返回随机返回0和1。 如果你只传递一个参数，那么将返回0和这个参数之间的整数。 如果两个参数相差值是1，随机返回传入的两个值。
#### 类型说明
::: info
