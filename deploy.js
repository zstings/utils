...
const status = shell.exec('git rev-list --left-right --count origin/docs...docs', { silent: true }).stdout.trim()
...
// 清空.docs目录 并 复制 docsDist 内容
shell.ls().forEach(file => {
  shell.rm('-rf', file)
})
console.log('[INFO] 清空 .docs 目录完成')
shell.cp('-r', `../docsDist/*`, './')
console.log('[INFO] 复制 docsDist 内容完成')
shell.exec('git add .')

shell.exec('git commit -m "deploy"')

shell.exec(`git push origin docs`)
